...
**Last refreshed:** 2026-07-26 · **Through step:** P2.10
...
Then skim [`docs/context.md`](../context.md) so you know **P3.1** is next and what is still a stub.
...
| 3 | [Module map](../manual/03-module-map.md) | Packages/files through P2.10 + stubs |
...
- **Readiness:** pure `compute_readiness` (P2 always `search_available=False`; unenriched limited-band needs `place_count >= 100` preferred — `>= 50` is seed volume only)  
- **Seeding:** `scripts/seed_destination.py` — `seed_destination` / `seed_destination_into` / `seed_places`; use `--radius 50` if you need ~100+ places for limited-band readiness  
- **Verification:** `tests/geo|destinations|places|scripts` + `scripts/test_p2_smoke.py`  
- **Not built yet:** planner, search, travel_engine logic (P3+)  
