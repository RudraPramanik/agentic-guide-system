...
LangGraph runtime hardening for P5: config-only ToolContext, single tool-execution pathway, explicit list-state accumulation, timeout `last_known_state`, read-only tools, unconditional stuck-detector, exact `langgraph` pin, documented REPLAN coarse-graining.
...
`docs/steps/step5.md` steps 5.9 and 5.11 MUST use this shape (no agent-side default `execute_tool`).
...
`ToolContext` MUST NOT expose callbacks or a writable `TravelState` reference for tools to mutate. Tools MUST return `ToolResult` only. `apply_tool_result` inside `tool_executor_node` MUST be the sole writer of planning state from tool outcomes. Step 5.1 language about “callbacks to mutate allowed TravelState fields” MUST NOT appear in `docs/steps/step5.md`.
