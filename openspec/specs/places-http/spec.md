...
Public places HTTP layer — `GET /api/v1/places` (paginated list by destination) and `GET /api/v1/places/{place_id}`. Router → `PlaceService` only; no repository or direct DB access in the router.
