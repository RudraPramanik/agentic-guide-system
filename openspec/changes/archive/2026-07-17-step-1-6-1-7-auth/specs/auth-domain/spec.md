...
`GET /api/v1/auth/callback` SHALL exchange the code, fetch userinfo, upsert the user, issue a JWT, return `ApiResponse[TokenResponse]`, and set httpOnly `wandr_token`. On Google `error` query param or any `WandrError` during the flow, it MUST redirect to `/auth/error` (never an unhandled 500 HTML page). Handlers MUST use a single `AuthService` instance per request.

#### Scenario: Happy-path cookie set
- **WHEN** callback completes successfully
- **THEN** response body includes access token and user, and `wandr_token` cookie is set httpOnly with SameSite=lax and secure only in production

#### Scenario: OAuth failure redirects
- **WHEN** token exchange raises `GoogleOAuthError`
