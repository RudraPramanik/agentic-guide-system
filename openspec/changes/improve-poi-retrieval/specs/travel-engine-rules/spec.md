...
- Structural: `VISIT_DURATION_BY_CATEGORY` covering all structural categories in use by places retrieval, including the original P2 set (`museum|viewpoint|monastery|attraction|park|trailhead`) **and** `cafe|restaurant|temple|historic|nature`, with `VISIT_DURATION_DEFAULT_MIN=30`, `MORNING_ONLY_CATEGORIES=["viewpoint"]` (no `sunrise_point`), `AVOID_SAME_DAY_PAIRS`
...
- **THEN** its keys are a superset of `{museum, viewpoint, monastery, attraction, park, trailhead, cafe, restaurant, temple, historic, nature}`

#### Scenario: Cafe has an explicit duration

- **WHEN** `visit_duration_min("cafe")` is called
- **THEN** the result is the configured cafe duration (not merely relying on an accidental default) and is a positive integer
