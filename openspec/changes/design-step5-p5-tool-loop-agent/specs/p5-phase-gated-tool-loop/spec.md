...
Nodes MUST call `execute_tool` only — never import tool implementation functions directly.
...
`ToolContext` MUST carry `destination_id`, `base_lat`, `base_lng`, `routing: RoutingProvider`, optional `db`, and access to mutable planning state helpers. `AsyncSession` and `RoutingProvider` MUST NOT be fields of LangGraph-checkpointed `TravelState`.

Tools that need DB access SHOULD acquire their own session (preferred) rather than holding one session for the full `PLANNER_GENERATION_TIMEOUT_SECONDS` window.
...
When the LLM returns no tool calls, the agent MUST: append a system nudge, retry once with `tool_choice="required"`, and if still none, execute the phase default tool (DISCOVER → `check_readiness`), recording the path in `tool_trace`.

Every successful `execute_tool` dispatch MUST increment `tool_loop_count` and append a `tool_trace` entry.
...
- **THEN** the phase default tool is executed and `tool_trace` records the nudge/default path
...
The compiled LangGraph MUST wire: `parse_preferences` → `agent` ↔ `tool_executor` loop, with conditionals for `needs_clarification` → END, `plan_complete` → `write_narrative` → `record_evaluation` → END, else loop to agent.

Graph compilation MUST succeed at import/startup (caught before first request). Package `langgraph` MUST be added to `requirements.txt` with a why-comment at the TravelState/graph step (5.6).
